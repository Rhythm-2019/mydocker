...
// --
var appConfig = &config{M: sync.Mutex{}}
...
func Init() {
    ...
    for k := 0; k < t.NumField(); k++ {
        defaultValue := t.Field(k).Tag.Get("default")
        if len(defaultValue) != 0 && v.Field(k).Kind() == reflect.String {
            v.Field(k).SetString(defaultValue)
        }
    }


    appConfig.CgroupRoot, err = toolkit.GetCgroupRoot()
    if err != nil {
...
func ImageInfoPath() string {
    return filepath.Join(StorePath(), "images.json")
}
func CgroupRootPath() string {
    return Config().CgroupRoot
}

func ContainerInfoPath() string {
    return filepath.Join(StorePath(), "containers.json")
}
